...
const DEFAULT_CFG = {lessons:8,duration:40,startH:7,startM:30,breakAfter:4,breakMins:20};
...
function Sidebar({view,setView,user,onLogout,logo}) {
  ...
  return (
    <div style={{width:215,background:"#1e3a5f",color:"white",display:"flex",flexDirection:"column",flexShrink:0,overflowY:"auto"}}>
      ...
      <div style={{padding:"12px 14px",borderTop:"1px solid rgba(255,255,255,0.1)"}}>
        ...
        <Btn onClick={onLogout} v="ghost" full style={{fontSize:11,padding:"6px 8px"}}>🔓 Logout</Btn>
      </div>
    </div>
  );
}
...
function LoginPage({users,setUsers,students,onLogin,logo}) {
  ...
  function doReset(){if(nPw.length<6){setFMsg({t:"Min 6 chars.",ok:false});return;} const u=users.find(x=>x.username===fUN||x.email===fUN); setUsers(p=>p.map(x=>x.id===u.id?{...x,password:nPw}:x)); setFMsg({t:"Password reset! Login now.",ok:true}); setTimeout(()=>{setForgot(false);setFStep("req");setFUN("");setFCode("");setNPw("");setGenCode(null);setFMsg({t:"",ok:true});},2000);}
...
function TimetablePage({students,staff,user}) {
  ...
  useEffect(()=>{{const d=load("tnks_tt2");if(d)setTt(d);}; {const d2=load("tnks_ttcfg");if(d2){setCfg(d2);setCfgEdit(d2);}}; {const d3=load("tnks_ttsetup");if(d3)setSetupData(d3);};},[]);
  useEffect(()=>{if(Object.keys(tt).length>0) save("tnks_tt2",tt);},[tt]);
  useEffect(()=>{save("tnks_ttsetup",setupData);},[setupData]);
  ...
  function saveCfg(){const u={lessons:parseInt(cfgEdit.lessons)||8,duration:parseInt(cfgEdit.duration)||40,startH:parseInt(cfgEdit.startH)||7,startM:parseInt(cfgEdit.startM)||30,breakAfter:parseInt(cfgEdit.breakAfter)||4,breakMins:parseInt(cfgEdit.breakMins)||20}; setCfg(u); save("tnks_ttcfg",u); setMsg("✅ Config saved!"); setTimeout(()=>setMsg(""),2500);}
  ...
  return (
    <div style={{padding:24}}>
      ...
      {tab==="config"&&<Card><div style={{fontWeight:"bold",color:"#1e3a5f",marginBottom:16,fontSize:14}}>⚙️ Timetable Configuration</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:14}}>
        {[["LESSONS PER DAY","lessons",4,12],["LESSON DURATION (min)","duration",20,90],["START HOUR (24hr)","startH",6,9],["START MINUTE","startM",0,59],["BREAK AFTER LESSON #","breakAfter",1,11],["BREAK DURATION (min)","breakMins",10,60]].map(([label,key,min,max])=><div key={key}><label style={{fontSize:11,fontWeight:"bold",color:"#374151",display:"block",marginBottom:3}}>{label}</label><input type="number" min={min} max={max} value={cfgEdit[key]} onChange={e=>setCfgEdit({...cfgEdit,[key]:e.target.value})} style={{width:"100%",border:"1.5px solid #e2e8f0",borderRadius:8,padding:"8px 12px",fontSize:13,fontFamily:F}}/></div>)}
      </div><div style={{marginTop:16,background:"#eff6ff",borderRadius:8,padding:"10px 14px",fontSize:12,color:"#1d4ed8"}}><b>Preview:</b> {cfgEdit.lessons} lessons × {cfgEdit.duration}min, start {String(cfgEdit.startH).padStart(2,"0")}:{String(cfgEdit.startM).padStart(2,"0")}, break {cfgEdit.breakMins}min after lesson {cfgEdit.breakAfter}</div>{msg&&<div style={{marginTop:10,fontSize:13,color:"#15803d",fontWeight:"bold"}}>{msg}</div>}<div style={{marginTop:14}}><Btn onClick={saveCfg} v="primary">💾 Save Config</Btn></div></Card>}
...
function DutyPage({staff,user}) {
  ...
  const [msg,setMsg]=useState({t:"",ok:true});
  useEffect(()=>{{const d=load("tnks_duties");if(d)setDuties(d);};},[]);
...
function CouncilPage({students,user}) {
  ...
  const [msg,setMsg]=useState({t:"",ok:true});
  useEffect(()=>{{const d=load("tnks_council");if(d)setCouncil(d);}; {const d2=load("tnks_studuties");if(d2)setStuDuties(d2);};},[]);
...
export default function App() {
  ...
  useEffect(()=>{
    (() => {
      const u=load("tnks_users"),s=load("tnks_students"),r=load("tnks_results"),c=load("tnks_comments"),a=load("tnks_announcements"),f=load("tnks_fees"),st=load("tnks_staff"),lg=load("tnks_logo");
      if(u) setUsers(u); if(s) setStudents(s); if(r) setResults(r);
      if(c) setComments(c); if(a) setAnnouncements(a); if(f) setFees(f); if(st) setStaff(st); if(lg) setLogo(lg);
      setReady(true);
    })();
  },[]);
